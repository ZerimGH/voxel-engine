...
static void update_camera_pos(Player *player) {
    ...
    player->camera->position[1] = player->position[1] - player->hitbox_dims[1] / 2.f + PLAYER_EYE_HEIGHT
...
static bool pos_collides_specific(vec3 pos, vec3 hitbox_dims, float bx, float by, float bz) {
    ...
    for (int x = xa; x <= xb; x++) {
        for (int y = ya; y <= yb; y++) {
            for (int z = za; z <= zb; z++) {
                ...
                if (pos_collides_specific(pos, hitbox_dims, (float)x, (float)y, (float)z)) {
                    return true;
                }
            }
        }
...
bool player_break(Player *player, World *world) {
    ...
    if (!player->selection.hit || !player->selection.block_hit) {
        return false;
    }
...
bool player_place(Player *player, World *world) {
    ...
    if (pos_collides_specific(player->position, player->hitbox_dims, player->selection.last_x, player->selection.last_y,
