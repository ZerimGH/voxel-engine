...
Crosshair *create_crosshair(void) {
  ...
  if(!texture) {
    fprintf(stderr, "(create_crosshair): Couldn't create crosshair, nu_load_texture() returned NULL.\n");
    return NULL;
  }
  Crosshair *c = calloc(1, sizeof(Crosshair));
  if(!c) {
    nu_destroy_texture(&texture);
    fprintf(stderr, "(create_crosshair): Couldn't create crosshair, calloc failed.\n");
    return NULL;
  }
  c->texture = texture;
  return c;
...
void render_crosshair(Crosshair *crosshair, UiRenderer *ui, float w, float h) {
  if(!crosshair || !ui) return; 
  // calculate position and size of crosshair 
  float screen_size = w/1920.f * CROSSHAIR_SIZE;
  float pos_x = w / 2.f - screen_size / 2.f;
  float pos_y = h / 2.f - screen_size / 2.f;
  nu_bind_texture(crosshair->texture, 0);
  ui_render_quad(ui, pos_x, pos_y, screen_size, screen_size, w, h);
}
