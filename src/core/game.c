...
Game *create_game(void) {
  ...
  if (!window) {
    sprintf(err_msg,
        "(create_game): Error creating game: nu_create_window() "
        "returned NULL.\n");
    goto failure;
  }
  ...
  if (!sky_renderer) {
    sprintf(err_msg,
        "(create_game): Error creating game: "
  ...
  if (!ui_renderer) {
    sprintf(err_msg,
        "(create_game): Error creating game: create_ui_renderer() "
        "returned NULL\n");
    goto failure;
  }
  ...
  if (!ui_renderer) {
    sprintf(err_msg,
        "(create_game): Error creating game: create_crosshair() "
        "returned NULL\n");
    goto failure;
  }
  ...
  if (!outline_renderer) {
    sprintf(err_msg,
        "(create_game): Error creating game: create_outline_renderer() returned "
        "NULL\n");
    goto failure;
  }
...
static void game_update_time(Game *game) {
  ...
  if (frames_passed >= FPS_RESET_FRAMES) {
    frames_passed = 0;
    dt_acc        = 0.f;
  }
...
void update_game(Game *game) {
  ...
  player_set_jumping(
      game->player, nu_get_key_state(game->window, GLFW_KEY_SPACE));
  player_set_crouching(
      game->player, nu_get_key_state(game->window, GLFW_KEY_LEFT_SHIFT));
  player_set_sprinting(game->player,
  ...
  player_rotate(game->player,
      nu_get_delta_mouse_x(game->window),
...
static void debug_print(Game *game, char *str, float *cur_y) {
  ...
  *cur_y -= size;
  *cur_y -= vertical_pad;
  text_render_string(game->text_renderer,
      game->ui_renderer,
      0,
      *cur_y,
      size,
...
static void render_debug(Game *game) {
  ...
  // Chunk info
  Chunk *chunk = world_get_chunkf(game->world,
      game->player->position[0],
  ...
  if (chunk) {
    ...
    sprintf(str,
        "  num vertices: %zu",
        chunk->mesh ? chunk->mesh->last_send_size / chunk->mesh->stride : 0);
    debug_print(game, str, &cur_y);
  }
}
...
void render_game(Game *game) {
  ...
  if (game->debug) { render_debug(game); }
