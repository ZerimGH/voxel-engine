...
Player *create_player(void) {
  ...
  if (!player) {
    fprintf(
        stderr, "(create_player): Error initialising player: calloc failed.\n");
    return NULL;
  }
  player->camera = create_camera(0.1, 512, WALKING_FOV);
  if (!player->camera) {
    fprintf(stderr,
        "(create_player): Error initialising player: "
...
static void update_camera_pos(Player *player) {
  ...
  player->camera->fov         = WALKING_FOV
                        + (SPRINTING_FOV - WALKING_FOV) * player->sprint_interp;
...
static bool pos_collides_specific(
    vec3 pos, vec3 hitbox_dims, float bx, float by, float bz) {
  ...
  return (min_x < bmax_x && max_x > bmin_x) && (min_y < bmax_y && max_y > bmin_y)
  ...
  for (int x = xa; x <= xb; x++) {
    for (int y = ya; y <= yb; y++) {
      for (int z = za; z <= zb; z++) {
        ...
        if (!block || block->type == BlockAir) { continue; }
        if (pos_collides_specific(
                pos, hitbox_dims, (float)x, (float)y, (float)z)) {
          return true;
        }
      }
    }
...
static void clamp_horizontal_velocity(Player *player) {
  ...
  if (player->is_sprinting) { max_speed *= PLAYER_SPRINT_MULT; }
  if (player->is_crouching) { max_speed *= PLAYER_CROUCH_MULT; }
...
static void player_apply_horizontal_movement(Player *player) {
  ...
  if (player->is_sprinting) { accel *= PLAYER_SPRINT_MULT; }
  if (player->is_crouching) { accel *= PLAYER_CROUCH_MULT; }
  ...
  for (int i = 0; i < 3; i++) {
    ...
    while (fabsf(remaining) > 0.f) {
      ...
      if (!pos_collides(test_pos, player->hitbox_dims, world)) {
        ...
        if (i != 1) { player->keep_sprint = false; }
        break;
      }
    }
  }
...
bool player_break(Player *player, World *world) {
  ...
  world_set_block(world,
      BlockAir,
      player->selection.hit_x,
      player->selection.hit_y,
      player->selection.hit_z);
  return true;
}
...
bool player_place(Player *player, World *world) {
  if (!player || !world) { return false; }
  if (!player->selection.hit || !player->selection.block_hit) { return false; }
  if (pos_collides_specific(player->position,
          player->hitbox_dims,
          player->selection.last_x,
          player->selection.last_y,
          player->selection.last_z)) {
    return false;
  }
