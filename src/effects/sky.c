...
SkyRenderer *create_sky_renderer(void) {
  nu_Program *program = nu_create_program(
      2, "shaders/sky.vert", "shaders/sky.frag");
  nu_Mesh *mesh = nu_create_mesh(
      sky_vertex_num, sky_vertex_sizes, sky_vertex_counts, sky_vertex_types);
  if (!program || !mesh) {
...
void render_sky(
    SkyRenderer *renderer, float screen_height, float pitch, float fov) {
  if (!renderer) { return; }
  // Set OpenGL stuff
