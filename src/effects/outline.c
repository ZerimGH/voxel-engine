...
OutlineRenderer *create_outline_renderer(void) {
  ...
  nu_Program *program = nu_create_program(
      2, "shaders/outline.vert", "shaders/outline.frag");
  if (!program) {
    fprintf(stderr,
        "(create_outline_renderer): Couldn't create outline renderer, "
  ...
  nu_Mesh *mesh = nu_create_mesh(
      outline_num, outline_sizes, outline_counts, outline_types);
  if (!mesh) {
  ...
  if (!outline) {
    fprintf(stderr,
        "(create_outline_renderer): Couldn't create outline renderer, calloc "
        "failed.\n");
    nu_destroy_program(&program);
