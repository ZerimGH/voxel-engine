...
OutlineRenderer *create_outline_renderer(void) {
    ...
    if (!program) {
        fprintf(stderr,
                "(create_outline_renderer): Couldn't create outline renderer, nu_create_program() returned NULL.\n");
        return NULL;
    }
    ...
    if (!mesh) {
        ...
        fprintf(stderr,
                "(create_outline_renderer): Couldn't create outline renderer, nu_create_mesh() returned NULL.\n");
        return NULL;
    }
...
void render_outline(OutlineRenderer *outline, Player *player, float width, float height) {
    ...
    // Set uniforms
    GLfloat uPos[3] = {(GLfloat)player->selection.hit_x, (GLfloat)player->selection.hit_y,
