...
static inline void emit_face(Vertex *target, size_t *count, float p[4][3], float s[4], float t[4], bool face_positive,
                             bool flip_winding, int side_index, int block_type) {
#define EMIT(i)                                                                                                        \
    target[(*count)++] = (Vertex) {                                                                                    \
        ...
        for (int slice = -1; slice < dims[axis]; slice++) {
            ...
            for (size_t y = 0; y < size_v; y++) {
                for (size_t x = 0; x < size_u; x++) {
                    ...
                    BlockType blockA = (coords[axis] >= 0 && coords[axis] < dims[axis])
                                           ? chunk->blocks[CHUNK_INDEX(coords[0], coords[1], coords[2])].type
                                           : BlockAir;

                    coords[axis] = slice + 1;
                    BlockType blockB = (coords[axis] >= 0 && coords[axis] < dims[axis])
                                           ? chunk->blocks[CHUNK_INDEX(coords[0], coords[1], coords[2])].type
                                           : BlockAir;
            ...
            while (y < size_v) {
                ...
                while (x < size_u) {
                    ...
                    while (y + height < size_v && !stop) {
                        for (size_t k = 0; k < width; k++) {
                            if (mask[(y + height) * size_u + x + k] != current
                                || face_positive_mask[(y + height) * size_u + x + k] != facePositive) {
                                stop = true;
                                break;
                            }
                        }
                    ...
                    float p[4][3] = {
                        ...
                        {base[0] + du[0] + dv[0] + corner_x, base[1] + du[1] + dv[1] + corner_y,
